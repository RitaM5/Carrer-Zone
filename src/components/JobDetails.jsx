...
const JobDetails = () => {
    ...
    useEffect(() => {
        if (data) {
            const jobDetailsData = data.find(detail => detail.id === id)
        ...
        <div className='my-container'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-20 mt-14'>
                <div className=' space-y-5 tracking-wider'>
                    ...
                    <p>
                        <span className='font-semibold text-lg mt-5'>Educational Requirements: </span>
                        <br />
                        {details.education}
                    </p>
                    <p>
                        <span className='font-semibold text-lg mt-5'>Experience: </span>
                        <br />
                        {details.experience}
                    </p>
                </div>
                {/* card */}
                <div className="card w-full h-full bg-base-100 shadow-2xl p-6 space-y-3">
                    <p className='text-2xl font-semibold my-4 ml-6'>Job Details</p>
                    <hr className='ml-6' />
                    <p className='inline-flex gap-2 ml-6'>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span className='font-bold'>Salary:</span>{details.salary}(Per Month)
                    </p>
                    <p className='inline-flex gap-2 ml-6'>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 14.15v4.25c0 1.094-.787 2.036-1.872 2.18-2.087.277-4.216.42-6.378.42s-4.291-.143-6.378-.42c-1.085-.144-1.872-1.086-1.872-2.18v-4.25m16.5 0a2.18 2.18 0 00.75-1.661V8.706c0-1.081-.768-2.015-1.837-2.175a48.114 48.114 0 00-3.413-.387m4.5 8.006c-.194.165-.42.295-.673.38A23.978 23.978 0 0112 15.75c-2.648 0-5.195-.429-7.577-1.22a2.016 2.016 0 01-.673-.38m0 0A2.18 2.18 0 013 12.489V8.706c0-1.081.768-2.015 1.837-2.175a48.111 48.111 0 013.413-.387m7.5 0V5.25A2.25 2.25 0 0013.5 3h-3a2.25 2.25 0 00-2.25 2.25v.894m7.5 0a48.667 48.667 0 00-7.5 0M12 12.75h.008v.008H12v-.008z" />
                        </svg>
                        <span className='font-bold'>Job Title:</span> {details.category}
                    </p>
